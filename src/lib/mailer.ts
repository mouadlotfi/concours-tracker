...
function buildNotifyHtml(concoursList: MatchedConcours[], unsubUrl: string): string {
  ...
  for (const c of concoursList) {
    ...
    const url = c.sourceUrl || c.wadifaUrl || '#';
    const reason = c.matchReason || '';
    const deadline = c.depositDeadlineIso
      ? `<p style="margin:6px 0 0;font-size:11px;font-family:${mono};color:${textDim};letter-spacing:0.3px;">DATE LIMITE &mdash; ${c.depositDeadlineIso}</p>`
      : '';
    itemsHtml += `
